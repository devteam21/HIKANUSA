package id.zapnut.project.HIKANUSA.Fragment;
...
import id.zapnut.project.HIKANUSA.Activity.MainActivity;
import id.zapnut.project.HIKANUSA.Activity.Search;
import id.zapnut.project.HIKANUSA.Adapter.LatestAdapterGV;
import id.zapnut.project.HIKANUSA.Adapter.LatestAdapterLV;
import id.zapnut.project.HIKANUSA.Item.SubCategoryList;
import id.zapnut.project.HIKANUSA.R;
import id.zapnut.project.HIKANUSA.Util.Constant_Api;
import id.zapnut.project.HIKANUSA.Util.Method;
...
public class LatestFragment extends Fragment {
    ...
    public void Latest(final boolean isTrue) {
        ...
        client.get(Constant_Api.latest, null, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        Constant_Api.subCategoryLists.add(new SubCategoryList(id, cat_id, book_title, book_description, Constant_Api.image + book_cover_img, Constant_Api.image + book_bg_img, book_file_type, total_rate, rate_avg, book_views, author_id, author_name));

                    }

                    if (isTrue) {
                        latestAdapterLV = new LatestAdapterLV(getActivity(), Constant_Api.subCategoryLists,"latest");
                        recyclerView.setAdapter(latestAdapterLV);
                        progressBar.hide();
                    } else {
                        latestAdapterGV = new LatestAdapterGV(getActivity(), Constant_Api.subCategoryLists,"latest");
